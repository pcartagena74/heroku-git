...
(function(factory) {
    ...
    EmailBuilder.prototype = {
        ...
        default_func: function() {
            ...
            _this.$elem.find('#button-text-color').ColorPicker({
                ...
                onChange: function(hsb, hex, rgb) {
                    ...
                    _this.changeSettings('font-color', '#' + hex);
                }
            });
            //button-bg-color
            ...
            //button-bg-color
            _this.$elem.find('#divider-color').ColorPicker({
                color: '#000',
                onChange: function(hsb, hex, rgb) {
                    $('#divider-color').css('background-color', '#' + hex);
                    _this.changeSettings($('#divider-color').attr('setting-type'), '#' + hex);
                }
            });
        ...
        events: function() {
            ...
            //master backgound predefined
            _this.$elem.find('#pre-define-bg-color .mce-grid-cell').on('click', function() {
            ...
            //button backgound predefined
            _this.$elem.find('#pre-define-button-bg-color .mce-grid-cell').on('click', function() {
                let color = $(this).find('div').css('background-color');
                $('#bg-color').css('background-color', '#' + color);
                _this.changeSettings($('#bg-color').attr('setting-type'), '#' + color);
            });
            //button font predefined
            _this.$elem.find('#pre-define-button-font-color .mce-grid-cell').on('click', function() {
                let color = $(this).find('div').css('background-color');
                $('#button-text-color').css('background-color', '#' + color);
                _this.changeSettings('font-color', '#' + color);
            });
            //divider color predefined
            _this.$elem.find('#pre-define-divider-color .mce-grid-cell').on('click', function() {
                let color = $(this).find('div').css('background-color');
                $('#divider-color').css('background-color', '#' + color);
                _this.changeSettings('divider-color', '#' + color);
            });
            //button-apply-all put same color to all button inside builder
            _this.$elem.find('#button-apply-all').on('click', function() {
                let text_color = $('#button-text-color').css('background-color');
                let bg_color = $('#bg-color').css('background-color');
                $('.email-editor-elements-sortable').find('a.button-1.hyperlink').css('background-color', bg_color);
                $('.email-editor-elements-sortable').find('a.button-1.hyperlink').css('color', text_color);
            });
        ...
        getSettings: function(_thisElement = null) {
            ...
            //button background
            _this.$elem.find('.tab-property .elements-accordion-item [setting-type="button-bg-color"]').css('background-color', _element.css('background-color'));
            //button font color 
            _this.$elem.find('.tab-property .elements-accordion-item [setting-type="button-text-color"]').css('background-color', _element.css('color'));
            //divider color
            _this.$elem.find('.tab-property .elements-accordion-item [setting-type="divider-color"]').css('background-color', _element.find('div').css('border-top-color'));
            /*Paddings*/
            ...
            if (_element.hasClass('button-1')) {
                ...
                //check box 
                let btn_width = _element.css('display');
                if (btn_width == 'block') {
                    _this.$elem.find('.button-full-width').attr('checked', 'true');
                } else {
                    _this.$elem.find('.button-full-width').removeAttr('checked');
                }
            }
            //image size
        ...
        changeSettings: function(type, value) {
            ...
            if (type == 'font-color' && _activeElement.hasClass('button-1')) {
                _activeElement.css('color', value);
            }
            if (type == 'divider-color' && _activeElement.hasClass('divider-simple')) {
                _activeElement.find('div').css('border-top-color', '#' + value)
            }
        },
        /**
    ...
    $.fn.emailBuilder = function(options) {
         ...
         * Set value for org info menu item in context menu
         */
        this.backgroundExtent = function(backgroundExtent) {
            _emailBuilder.config.backgroundExtent = backgroundExtent;
        }
        /**
