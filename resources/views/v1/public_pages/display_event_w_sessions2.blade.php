                                    ...
                                    <td style="text-align: center;">
                                        <div class="form-group">
                                            <input type="number" pattern="[0-5]" name="q-{{ $bundle->ticketID }}"
                                                   ...
                                                   value="0" required data-error="{{ trans('messages.errors.numeric') }}">
...
?>
                                ...
                                <tr>
                                    ...
                                    <td data-title="{{ trans('messages.fields.ticket') }}">{{ $ticket->ticketLabel }}
                                        @if($ticket->maxAttendees > 0 && $ticket->regCount >= $ticket->maxAttendees)
                                            <br /><b class="red">@lang('messages.instructions.sold_out')</b>
                                        @endif
                                    </td>
                                    <td data-title="{{ trans('messages.fields.memprice') }}">
                                        @lang('messages.symbols.cur')
                    ...
                    <div class="col-md-6 col-sm-6 col-xs-12">
                        ...
                        <div class="col-md-3 col-sm-3 col-xs-6"><a class="btn btn-xs btn-primary"
                                                                   id="btn-validate">@lang('messages.fields.validate')</a></div>
                    </div>
...
?>
                                                    @if($s !== null)
                                                        @if($tracks->first() == $track || !$event->isSymmetric)
                                                            <td data-title="{{ trans('messages.fields.times') }}" rowspan="3" style="text-align:left;">
                                                                <nobr> {{ $s->start->format('g:i A') }} </nobr>
                                                                &dash;
                                                                <nobr> {{ $s->end->format('g:i A') }} </nobr>
                                                            </td>
                                                        @endif
                                                        <td data-title="{{ trans('messages.tabs.session') }}" colspan="2"
                                                            ...
                                                            <a tabindex="0" class="btn btn-xs btn-primary pull-right"
                                                               ...
                                                               data-content="{!! $s->sessionAbstract !!}">@lang('messages.fields.abstract')</a><br/>
                                                            @endif
                                                        </td>
                                                    @else
...
?>
                                                    @if($s !== null && $s->sessionSpeakers !== null)
                                                        <td data-title="Speaker" colspan="2" style="text-align:left;">
                                                            ...
                                                            {{ $s->sessionSpeakers or trans('messages.fields.tbd') }}
                                                        </td>
                                                    @else
                                                        &nbsp;
                                                    @endif
...
?>
                                                    @if($s !== null && $s->leadAmt+$s->stratAmt+$s->techAmt > 0 && $s->maxAttendees > 0)
                                                        <td data-title="{{ trans('messages.fields.credit') }}" style="text-align:left;">
                                                            @if($s->leadAmt > 0)
                                                                <b>{{ $s->leadAmt }}
                                                        ...
                                                        <td data-title="{{ trans('messages.fields.limit') }}" style="text-align:left;">
    ...
    <div class="col-md-12 col-sm-12 col-xs-12 form-group has-feedback">
        <table class="table" style="border: none;">
            <tr style="border: none;">
                <td style="text-align: right; border: none;"><h4 class="red">@lang('messages.fields.from'):</h4></td>
    ...
    @if($event->eventInfo)
        @include('v1.parts.start_content', ['header' => trans('messages.fields.additional'), 'subheader' => '', 'w1' => '8', 'w2' => '12', 'r1' => 0, 'r2' => 0, 'r3' => 0])
        {!! $event->eventInfo !!}
        @if(isset($tags))
